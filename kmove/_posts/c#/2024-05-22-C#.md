...
<h4> 11-2. Program 클래스가 속한 네임스페이스 이름은 ( ConsoleApp1 )이다. </h4>


<h4> 11-3. Console.WriteLine()은 ( System ) 네임스페이스에 속한 클래스 메소드이다. </h4>


<h4> 11-4. 클래스 이름은 ( Program )이며 멤버함수로 ( Print() )이 있다. </h4>
   
   
<h4> 11-5. 만약 여기에서 using System이 없다면 Console.WriteLine(”Hello World!)을 어떻게 수정해야 하는가? </h4>
...
<h4> 1-1.  MathOperations 네임스페이스에는 ( Calculator ) ( Geometry )( Program ) 클래스가 있다. </h4>
   
<h4> 1-2. 이 소스에서 namespace는 ( MathOperations )이다. </h4>
   
<h4> 1-3. 하나의 네임스페이스에는 ( 여러개 )의 클래스가 있다. </h4>

<h4> 1-4. Calculator클래스의 메소드에는 ( Add )( Subtract )이 있다. </h4>

<h4> 1-5. Geometry클래스에서도 Main()메소드를 추가할수 있다( X ) </h4>

<h4> 1-6. Main메소드에서 Calculator.Add()라고 using이 없는데 네임스페이스명을 사용하지 않는 이유는 무엇이라고 생각하는가? </h3>
...
<h4> 1-7. Main메소드에서 Calculator.Add()에서 클래스명.메소드로 호출했는데 그 이유가 무엇인가? </h4>
...
<h4> 2-1. ①번의 경우 MathOperaions.Calculator.Add(5,3)의 코드에서  MathOperaions는 ( 네임스페이스 )이고 Calculator는 ( 클래스 )이고 .Add는 ( 메소드 )이고 5,3은 ( 매개변수 )이다. </h4>

<h4> 2-2. ①번의 답을 통해 네임스페이스의 클래스의 멤버를 접근하기 위해서는 어떠한 문법형태를 가져야 하는지를 쓰시오 </h4>
...
<h4> 2-3. MathOperations의 네임스페이스가 여러군데 반복하여 작성되고 있다. 이러한 번거로움을 제거하기 위한 방법을 쓰시오 </h4>
...
<h3> 3. 아래에는 네임스페이스가 다른데 같은 이름을 갖는 클래스를 호출하는 예제이다. </h3>
...
<h4> 3-1. 이 예제는 MathOperaions과 OtherNamespace에서 모두 클래스 이름이 ( Calculator ) 로 동일하다. </h4>

<h4> 3-2. 이런 경우 MathOperaions 네임스페이스의 Calculator클래스의 Add메소드를 호출하고 싶으면 어떤 코드로 호출했는가? </h4>
...
<h4> 3-3. 이 코드를 보고 클래스 이름이 같지만 네임스페이스가 다른 경우 네임스페이스명을 반드시 코드에 작성해야 한다는 것을 알았다( O ) </h4>


<h3> 4. 아래 빈칸에 들어갈 명령문은? </h3>
